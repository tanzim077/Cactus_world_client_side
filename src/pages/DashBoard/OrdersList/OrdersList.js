...
const OrdersList = () => {
  ...
  const handleStatus = (id) => {
    ...
    axios
      .put(`http://localhost:8080/orders/status/${id}`, newData)
          ...
          <tbody>
            {data.map((d) => (
              <tr>
                ...
                <td>{d.price}</td>
                <td>
                  <img className="img-thumbnail" src={d.image} alt="" />
                </td>
                <td>{d.bookedDate}</td>
                <td>
                  {d.userStatus === "pending" ? (
                    <Button onClick={() => handleStatus(d._id)} variant="info">
                      Shipped 🚛
                    </Button>
                  ) : (
                    <Button
                ...
                <td>
                  <Button onClick={() => handleDelete(d._id)} variant="danger">
                    Delete
                  </Button>
                </td>
              </tr>
